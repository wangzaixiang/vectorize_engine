...
use std::simd::{simd_swizzle, u32x16, u32x4, u32x8, ToBytes};
use rand::random;
...
pub fn sort_u32x16(nums: &mut u32x16) {
    ...
    let b = simd_swizzle!(min, max, [4, 12, 5, 13, 6, 14, 7, 15]);  // b = [4, 12, 3, 11, 2, 10, 1, 9]
    // end 1v1 * 8 group, now a, b is 2v2 * 4 group

    let min: u32x8 = a.simd_min(b);    // [2,6,1,5]
    let max: u32x8 = a.simd_max(b);    // [4,8,3,7]
